                    ...
                    if (ind.search(' ' + index.toString() + ' ') >= 0) {
                        ...
                        val.user.length > 0 && val.user.forEach((element, indx) => {
                            ...
                            setpreparedata(pr => [...pr, {
                                ...
                                OT_Pay_rate: Number( element.otpayrate) + Number(element.otpayrate) * Number(val.markup) / 100,
                                nc_4: element.nc === 'no' ? '-' : ((Number(element.payrate) * 0) + (0 * Number(element.otpayrate))) * 4 / 100,
                                total: (Number(element.payratetype==='custom'?element.cpr:element.payrate) * (allhours.find(obj => obj.userid === element.userid&& obj.siteid === val._id)?.Hrs)||0) + ((allhours.find(obj => obj.userid === element.userid&& obj.siteid === val._id)?.Ot_Hrs)||0 * Number(element.otpayrate)),
                                deductions: 0,
                                net: (Number(element.payratetype==='custom'?element.cpr:element.payrate) * (allhours.find(obj => obj.userid === element.userid&& obj.siteid === val._id)?.Hrs)||0) + ((allhours.find(obj => obj.userid === element.userid&& obj.siteid === val._id)?.Ot_Hrs)||0 * Number(element.otpayrate)) - 0 - (element.nc === 'no' ? 0 : ((Number(element.payrate) * 0) + (0 * parseInt(element.otpayrate))) * 4 / 100)
                        ...
                        {k == 1 && l === 2 &&
                            <div className='sssw' style={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
                                ...
                                <div className="subadduser subadduserx2">
                                    ...
                                    <>
                                        <div className="inputname">
                                            <h1>Company </h1>   <select className='select2 inject' name="cars" id="cars" onChange={e => selectthiscompany(e.target.value)}>
...
}
{
    data && data.map((val,index) => (
        ...
        <option value={`${val.clientid+'euika'+index}`}>{val.sitename}</option>
                                        ...
                                        <div className="inputname">
                                            <h1>Invoice #</h1>
                                            <input type="text"    onChange={e => setinno(e.target.value)} value={inno} />
                                        </div>
...
{l===2?
<input className='hrsedit' type="text" value={tempjson.Payrate}  onChange={e => settempjson(tempjson => ({
                                    ...tempjson,
...
<input className='hrsedit' type="text" value={tempjson.cpr}  onChange={e => settempjson(tempjson => ({
                                    ...tempjson,
...
<h4 style={{ width: '80px', marginBottom: '0px' }}  >$     <input className='hrsedit' type="text" value={tempjson.OT_Pay_rate}  onChange={e => settempjson(tempjson => ({
                                                                    ...tempjson,
...
<h4 style={{ width: '80px', marginBottom: '0px' }}  >
    $   <input className='hrsedit' type="text" value={tempjson.deductions}  onChange={e => settempjson(tempjson => ({
                                                                    ...tempjson,
